...
from despensa.controller.controller import Controller
from despensa.objects.classes import Recipe, Aliment
from despensa.utils.logger import Logger, LOG_API_REST
...
def get_all_aliments() -> Response:
    ...
    logger.info(
        f"(get_all_aliments) Output: Sended {len(aliments)} aliments: {[al.name for al in aliments]}"
    )
...
def update_aliment(aliment_id: str) -> Response:
    ...
    logger.info("(update_aliment) Output: Success")
...
def delete_aliment(aliment_id: int) -> Response:
    ...
    logger.info("(delete_aliment) Output: Success")
    return jsonify(success=True)
...
def get_all_recipes() -> Response:
    logger.info("(get_all_recipes)")
    recipes: list[Recipe] = Controller().get_recipes_catalog()
    logger.info(
        f"(get_all_recipes) Output: Sended {len(recipes)} recipes: {[re.name for re in recipes]}"
    )
...
def update_recipe(recipe_id: str) -> Response:
    ...
    logger.info("(update_recipe) Output: Success")
    return jsonify(success=True)
...
def delete_recipe(recipe_id: int) -> Response:
    ...
    logger.info("(delete_recipe) Output: Success")
    return jsonify(success=True)
...
def get_pantry() -> Response:
    logger.info("(get_pantry)")
    pantry = Controller().get_pantry()
    logger.info(
        f"(get_pantry) Output: Sended {len(pantry)} aliments in pantry: {[al.name for al in pantry]}"
    )
...
def add_to_pantry(name: str) -> Response:
    ...
    logger.info("(add_to_pantry) Output: Success")
    return jsonify(success=True)
...
def remove_from_pantry(name: str):
    ...
    logger.info("(remove_from_pantry) Output: Success")
    return jsonify(success=True)
...
def get_shopping_list():
    logger.info("(get_shopping_list)")
    shopping_list = Controller().get_shopping_list()
    logger.info(
        f"(get_shopping_list) Output: Sended {len(shopping_list)} items in shopping_list: {[it.name for it in shopping_list]}"
    )
...
def add_to_shopping_list(item: str):
    ...
    logger.info("(add_to_shopping_list) Output: Success")
    return jsonify(success=True)
...
def remove_from_shopping_list(item: str):
    ...
    logger.info("(remove_from_shopping_list) Output: Success")
    return jsonify(success=True)
